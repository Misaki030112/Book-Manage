...
import com.book.domain.BookAddCommand;
import com.book.service.BookService;
import com.book.domain.DefaultTairManager;
import com.book.domain.Result;
import com.book.domain.Tair;
...
public class BookController {
    ...
    // 初始化 tair 缓存对象（调用了构造函数，完成了初始化工作）和 DefaultTairManager默认管理器
    private final Tair tair = new Tair();
    private final DefaultTairManager defaultTairManager = tair.getDefaultTairManager();
    ...
    public String readerQueryBookDo(HttpServletRequest request,String searchWord,RedirectAttributes redirectAttributes){
        ...
        // 查询图书时先使用Tair缓存
        // 获取值，参数 命名空间/key
        Result result = defaultTairManager.get(0,searchWord);

        // Tair缓存找到结果，就使用Tair缓存的结果
        if(!(result.getMessage().equals("data not exist")))
        {
            redirectAttributes.addFlashAttribute("books", result.getBooks());
        ...
        if (exist){
            ArrayList<Book> books = bookService.queryBook(searchWord);
            // 加入缓存
            defaultTairManager.put(0,searchWord, books);
